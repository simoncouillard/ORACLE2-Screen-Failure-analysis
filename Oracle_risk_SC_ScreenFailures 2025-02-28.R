...
# Run the multivariable model with our new grouping variable
res_comb <- NULL
R_2 <- c()
...
# Create prediction datasets - one for each group
pred_data_high_low <- data.frame(
  high_FEV1_low_rev_group = "High FEV1, Low Rev",
  ACQ_baseline_score_mean_imputated = mean(Data_Oracle_filtered$ACQ_baseline_score_mean_imputated, na.rm = TRUE),
  Gender_imputated = names(which.max(table(Data_Oracle_filtered$Gender_imputated))),
  Age_imputated = mean(Data_Oracle_filtered$Age_imputated, na.rm = TRUE),  # Include age
  Treatment_step = names(which.max(table(Data_Oracle_filtered$Treatment_step))),
  Number_severe_attack_previous_12m = mean(Data_Oracle_filtered$Number_severe_attack_previous_12m, na.rm = TRUE),
  FEV1_preBD_PCT_Baseline_imputated = mean(Data_Oracle_filtered$FEV1_preBD_PCT_Baseline_imputated, na.rm = TRUE),
  FEV1_reversibility_percent_postBD_real = mean(Data_Oracle_filtered$FEV1_reversibility_percent_postBD_real, na.rm = TRUE),
  FEV1_FVC_ratio_imputated = mean(Data_Oracle_filtered$FEV1_FVC_ratio_imputated, na.rm = TRUE),
  FeNO_Log_imputated = mean(Data_Oracle_filtered$FeNO_Log_imputated, na.rm = TRUE),
  Eosinophils_Log_imputated = mean(Data_Oracle_filtered$Eosinophils_Log_imputated, na.rm = TRUE),
  Follow_up_duration_days = 365,  # 1 year follow-up for rate calculation
  Enrolled_Trial_name = trials_with_high_fev1_low_rev[1]  # Use first trial with high FEV1, low rev patients
)
...
# Modified function for prediction with more reasonable confidence intervals
predict_with_reasonable_CIs <- function(model_list, newdata) {
  # Get predictions from each imputation
  preds <- sapply(model_list, predict, newdata = newdata, type = "response")
  
  # Calculate the point estimate (mean across imputations)
  mean_pred <- mean(preds)
  
  # Use the variability between imputations as our uncertainty measure
  # Add a small correction factor based on sample size
  between_var <- var(preds)
  
  # For negative binomial, add a small component of variance based on mean
  # This is much more conservative than full theoretical variance
  model_var <- 0.05 * mean_pred  # 5% of mean as variance component
  
  # Total variance 
  total_var <- between_var + model_var
  
  # Standard error
  se <- sqrt(total_var)
  
  # 95% CI
  lower <- max(0, mean_pred - 1.96 * se)  # Prevent negative rates
  upper <- mean_pred + 1.96 * se
  
  return(c(mean_pred, lower, upper))
}

# Apply the modified function to get predictions
predicted_rates[1, 2:4] <- predict_with_reasonable_CIs(res_comb, pred_data_high_low)
predicted_rates[2, 2:4] <- predict_with_reasonable_CIs(res_comb, pred_data_other)

# Calculate rate ratio and its confidence interval
log_ratio <- log(predicted_rates$Rate[1]) - log(predicted_rates$Rate[2])
log_ratio_se <- sqrt(
  (log(predicted_rates$Upper_CI[1]/predicted_rates$Rate[1])/(1.96))^2 + 
    (log(predicted_rates$Upper_CI[2]/predicted_rates$Rate[2])/(1.96))^2
)

rate_ratio <- exp(log_ratio)
rate_ratio_lower <- exp(log_ratio - 1.96 * log_ratio_se)
rate_ratio_upper <- exp(log_ratio + 1.96 * log_ratio_se)

# Print results for primary analysis
cat("\n=== PRIMARY ANALYSIS: HIGH FEV1 & LOW REV ===\n")
print(res_pool)
cat("\nAdjusted Annual Asthma Attack Rates:\n")
print(predicted_rates)
cat(paste0("\nRate Ratio (High FEV1, Low Rev vs. Other): ", 
...
#########################################
# SENSITIVITY ANALYSIS 1 - HIGH FEV1 ONLY
#########################################

# Run model for High FEV1 Only
sensitivity_high_fev1 <- NULL
R_2_high_fev1 <- c()

for(i in 1:10){
  # Fit the model
  sensitivity_high_fev1[[i]] <- glm.nb(Number_severe_asthma_attacks_during_followup ~ 
                                         high_FEV1_group +
                                         ACQ_baseline_score_mean_imputated + 
                                         Gender_imputated +
                                         Age_imputated +
  ...
  R_2_high_fev1 <- c(R_2_high_fev1, with(summary(sensitivity_high_fev1[[i]]), 1 - deviance/null.deviance))
}

R2_mean_high_fev1 <- mean(R_2_high_fev1)
res_pool_high_fev1 <- summary(pool(sensitivity_high_fev1), conf.int = TRUE, exp = TRUE)

# Create prediction datasets for High FEV1 Only
pred_data_high_fev1 <- data.frame(
  high_FEV1_group = "High FEV1 Only",
  ACQ_baseline_score_mean_imputated = mean(Data_Oracle_filtered$ACQ_baseline_score_mean_imputated, na.rm = TRUE),
  Gender_imputated = names(which.max(table(Data_Oracle_filtered$Gender_imputated))),
  Age_imputated = mean(Data_Oracle_filtered$Age_imputated, na.rm = TRUE),
  Treatment_step = names(which.max(table(Data_Oracle_filtered$Treatment_step))),
  Number_severe_attack_previous_12m = mean(Data_Oracle_filtered$Number_severe_attack_previous_12m, na.rm = TRUE),
  FEV1_reversibility_percent_postBD_real = mean(Data_Oracle_filtered$FEV1_reversibility_percent_postBD_real, na.rm = TRUE),
  FEV1_FVC_ratio_imputated = mean(Data_Oracle_filtered$FEV1_FVC_ratio_imputated, na.rm = TRUE),
...
pred_data_not_high_fev1 <- pred_data_high_fev1
pred_data_not_high_fev1$high_FEV1_group <- "Not High FEV1"

# Make predictions for High FEV1 Only
predicted_rates_high_fev1 <- data.frame(
  Group = c("High FEV1 Only", "Not High FEV1"),
  Rate = numeric(2),
  Lower_CI = numeric(2),
  Upper_CI = numeric(2)
)
...
predicted_rates_high_fev1[1, 2:4] <- predict_with_reasonable_CIs(sensitivity_high_fev1, pred_data_high_fev1)
predicted_rates_high_fev1[2, 2:4] <- predict_with_reasonable_CIs(sensitivity_high_fev1, pred_data_not_high_fev1)

# Calculate rate ratio for High FEV1 Only
log_ratio_high_fev1 <- log(predicted_rates_high_fev1$Rate[1]) - log(predicted_rates_high_fev1$Rate[2])
log_ratio_se_high_fev1 <- sqrt(
  (log(predicted_rates_high_fev1$Upper_CI[1]/predicted_rates_high_fev1$Rate[1])/(1.96))^2 + 
    (log(predicted_rates_high_fev1$Upper_CI[2]/predicted_rates_high_fev1$Rate[2])/(1.96))^2
)

rate_ratio_high_fev1 <- exp(log_ratio_high_fev1)
rate_ratio_high_fev1_lower <- exp(log_ratio_high_fev1 - 1.96 * log_ratio_se_high_fev1)
rate_ratio_high_fev1_upper <- exp(log_ratio_high_fev1 + 1.96 * log_ratio_se_high_fev1)

# Print results for sensitivity analysis 1
cat("\n=== SENSITIVITY ANALYSIS 1: HIGH FEV1 ONLY ===\n")
print(res_pool_high_fev1)
cat("\nAdjusted Annual Asthma Attack Rates:\n")
print(predicted_rates_high_fev1)
cat(paste0("\nRate Ratio (High FEV1 Only vs. Not High FEV1): ", 
           round(rate_ratio_high_fev1, 2), " (95% CI: ", 
           round(rate_ratio_high_fev1_lower, 2), "-", 
           round(rate_ratio_high_fev1_upper, 2), ")"))


# Create Table 1 for sensitivity analysis: High FEV1 Only
data_imp1_high_fev1 <- subset(Data_Oracle_filtered, 
                              .imp == 1 & 
                                !is.na(high_FEV1_group))

table1_high_fev1 <- CreateTableOne(vars = vars_to_compare, 
                                   strata = "high_FEV1_group", 
                                   data = data_imp1_high_fev1, 
                                   addOverall = TRUE)

table1_high_fev1_summary <- print(table1_high_fev1, showAllLevels = TRUE, quote = FALSE, pDigits = 3, printToggle = FALSE)

cat("\nTable 1 for Sensitivity Analysis: High FEV1 Only\n")
kable(table1_high_fev1_summary, format = "markdown", caption = "Sensitivity Analysis 1: Comparison by High FEV1 status")


#########################################
# SENSITIVITY ANALYSIS 2 - LOW REV ONLY
#########################################

# Run model for Low Rev Only
sensitivity_low_rev <- NULL
R_2_low_rev <- c()

for(i in 1:10){
  # Fit the model
  sensitivity_low_rev[[i]] <- glm.nb(Number_severe_asthma_attacks_during_followup ~ 
                                       low_rev_group +
                                       ACQ_baseline_score_mean_imputated + 
                                       Gender_imputated +
                                       Age_imputated +
  ...
  R_2_low_rev <- c(R_2_low_rev, with(summary(sensitivity_low_rev[[i]]), 1 - deviance/null.deviance))
}

R2_mean_low_rev <- mean(R_2_low_rev)
res_pool_low_rev <- summary(pool(sensitivity_low_rev), conf.int = TRUE, exp = TRUE)

# Create prediction datasets for Low Rev Only
pred_data_low_rev <- data.frame(
  low_rev_group = "Low Rev Only",
  ACQ_baseline_score_mean_imputated = mean(Data_Oracle_filtered$ACQ_baseline_score_mean_imputated, na.rm = TRUE),
  Gender_imputated = names(which.max(table(Data_Oracle_filtered$Gender_imputated))),
  Age_imputated = mean(Data_Oracle_filtered$Age_imputated, na.rm = TRUE),
  Treatment_step = names(which.max(table(Data_Oracle_filtered$Treatment_step))),
...
pred_data_not_low_rev <- pred_data_low_rev
pred_data_not_low_rev$low_rev_group <- "Not Low Rev"

# Make predictions for Low Rev Only
predicted_rates_low_rev <- data.frame(
  Group = c("Low Rev Only", "Not Low Rev"),
  Rate = numeric(2),
  Lower_CI = numeric(2),
  Upper_CI = numeric(2)
)
...
# Apply prediction function
predicted_rates_low_rev[1, 2:4] <- predict_with_reasonable_CIs(sensitivity_low_rev, pred_data_low_rev)
predicted_rates_low_rev[2, 2:4] <- predict_with_reasonable_CIs(sensitivity_low_rev, pred_data_not_low_rev)

# Calculate rate ratio for Low Rev Only
log_ratio_low_rev <- log(predicted_rates_low_rev$Rate[1]) - log(predicted_rates_low_rev$Rate[2])
log_ratio_se_low_rev <- sqrt(
  (log(predicted_rates_low_rev$Upper_CI[1]/predicted_rates_low_rev$Rate[1])/(1.96))^2 + 
    (log(predicted_rates_low_rev$Upper_CI[2]/predicted_rates_low_rev$Rate[2])/(1.96))^2
)

rate_ratio_low_rev <- exp(log_ratio_low_rev)
rate_ratio_low_rev_lower <- exp(log_ratio_low_rev - 1.96 * log_ratio_se_low_rev)
rate_ratio_low_rev_upper <- exp(log_ratio_low_rev + 1.96 * log_ratio_se_low_rev)

# Print results for sensitivity analysis 2
cat("\n=== SENSITIVITY ANALYSIS 2: LOW REV ONLY ===\n")
print(res_pool_low_rev)
cat("\nAdjusted Annual Asthma Attack Rates:\n")
print(predicted_rates_low_rev)
cat(paste0("\nRate Ratio (Low Rev Only vs. Not Low Rev): ", 
           round(rate_ratio_low_rev, 2), " (95% CI: ", 
           round(rate_ratio_low_rev_lower, 2), "-", 
           round(rate_ratio_low_rev_upper, 2), ")"))

# Create Table 1 for sensitivity analysis: Low Rev Only
data_imp1_low_rev <- subset(Data_Oracle_filtered, 
                            .imp == 1 & 
                              !is.na(low_rev_group))

table1_low_rev <- CreateTableOne(vars = vars_to_compare, 
                                 strata = "low_rev_group", 
                                 data = data_imp1_low_rev, 
                                 addOverall = FALSE)

table1_low_rev_summary <- print(table1_low_rev, showAllLevels = TRUE, quote = FALSE, pDigits = 3, printToggle = FALSE)

cat("\nTable 1 for Sensitivity Analysis: Low Rev Only\n")
kable(table1_low_rev_summary, format = "markdown", caption = "Sensitivity Analysis 2: Comparison by Low Reversibility status")

#########################################
# SENSITIVITY ANALYSIS 3 - PRIMARY MODEL WITHOUT LUNG FUNCTION VARIABLES
#########################################

# Run model without FEV1, FEV1/FVC, and reversibility
sensitivity_no_lung_function <- NULL
R_2_no_lung_function <- c()

for(i in 1:10){
  # Fit the model without lung function variables
  sensitivity_no_lung_function[[i]] <- glm.nb(Number_severe_asthma_attacks_during_followup ~ 
                                                high_FEV1_low_rev_group +
                                                ACQ_baseline_score_mean_imputated + 
                                                Gender_imputated +
                                                Age_imputated +
  ...
  R_2_no_lung_function <- c(R_2_no_lung_function, with(summary(sensitivity_no_lung_function[[i]]), 1 - deviance/null.deviance))
}

R2_mean_no_lung_function <- mean(R_2_no_lung_function)
res_pool_no_lung_function <- summary(pool(sensitivity_no_lung_function), conf.int = TRUE, exp = TRUE)

# Create prediction datasets for model without lung function variables
pred_data_no_lung_high_fev1_low_rev <- data.frame(
  high_FEV1_low_rev_group = "High FEV1, Low Rev",
  ACQ_baseline_score_mean_imputated = mean(Data_Oracle_filtered$ACQ_baseline_score_mean_imputated, na.rm = TRUE),
  Gender_imputated = names(which.max(table(Data_Oracle_filtered$Gender_imputated))),
  Age_imputated = mean(Data_Oracle_filtered$Age_imputated, na.rm = TRUE),
...
pred_data_no_lung_other <- pred_data_no_lung_high_fev1_low_rev
pred_data_no_lung_other$high_FEV1_low_rev_group <- "Other"

# Make predictions for model without lung function variables
predicted_rates_no_lung_function <- data.frame(
  Group = c("High FEV1, Low Rev", "Other"),
  Rate = numeric(2),
  Lower_CI = numeric(2),
  Upper_CI = numeric(2)
...
# Apply prediction function
predicted_rates_no_lung_function[1, 2:4] <- predict_with_reasonable_CIs(sensitivity_no_lung_function, pred_data_no_lung_high_fev1_low_rev)
predicted_rates_no_lung_function[2, 2:4] <- predict_with_reasonable_CIs(sensitivity_no_lung_function, pred_data_no_lung_other)

# Calculate rate ratio for model without lung function variables
log_ratio_no_lung_function <- log(predicted_rates_no_lung_function$Rate[1]) - log(predicted_rates_no_lung_function$Rate[2])
log_ratio_se_no_lung_function <- sqrt(
  (log(predicted_rates_no_lung_function$Upper_CI[1]/predicted_rates_no_lung_function$Rate[1])/(1.96))^2 + 
    (log(predicted_rates_no_lung_function$Upper_CI[2]/predicted_rates_no_lung_function$Rate[2])/(1.96))^2
)

rate_ratio_no_lung_function <- exp(log_ratio_no_lung_function)
rate_ratio_no_lung_function_lower <- exp(log_ratio_no_lung_function - 1.96 * log_ratio_se_no_lung_function)
rate_ratio_no_lung_function_upper <- exp(log_ratio_no_lung_function + 1.96 * log_ratio_se_no_lung_function)

# Print results for sensitivity analysis 3
cat("\n=== SENSITIVITY ANALYSIS 3: PRIMARY MODEL WITHOUT LUNG FUNCTION VARIABLES ===\n")
print(res_pool_no_lung_function)
cat("\nAdjusted Annual Asthma Attack Rates:\n")
print(predicted_rates_no_lung_function)
cat(paste0("\nRate Ratio (High FEV1, Low Rev vs. Other): ", 
           round(rate_ratio_no_lung_function, 2), " (95% CI: ", 
           round(rate_ratio_no_lung_function_lower, 2), "-", 
           round(rate_ratio_no_lung_function_upper, 2), ")"))

# Compare R-squared values
cat("\n\nR-squared comparison:\n")
cat("Primary model R-squared:", round(R2_mean, 4), "\n")
cat("No lung function variables R-squared:", round(R2_mean_no_lung_function, 4), "\n")
cat("Difference:", round(R2_mean - R2_mean_no_lung_function, 4), "\n")